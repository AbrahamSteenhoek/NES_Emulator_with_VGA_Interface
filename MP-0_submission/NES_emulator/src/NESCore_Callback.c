...
void ChangePixel( uint16_t *img, const uint16_t pixel, const u32 row, const u32 col )
{
	const u32 width = WIDTH;
	img[ width * row + col ] = pixel;
}
...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
	...
	for (i = 0 ; i < NES_DISP_HEIGHT; i++) {
		...
		for (j = 0; j < NES_DISP_WIDTH; j++) {
			const uint16_t VGA_col = (j * 2) + 64;
			const uint16_t VGA_row = (i * 2);

			// Grab a temporary pixel using the color palette lookup table.
			tpixel = NesPalette3[WorkFrame[NES_DISP_WIDTH*i+j]];
			uint16_t tpix_shifted = tpixel << 4;

			//ChangePixel( ptr, tpixel, i, j );
			ptr[ 640 * VGA_row + VGA_col ] = tpix_shifted;
			ptr[ 640 *  (VGA_row + 1 ) + VGA_col ] = tpix_shifted;
			ptr[ 640 * VGA_row + ( VGA_col + 1 ) ] = tpix_shifted;
			ptr[ 640 * ( VGA_row + 1 ) + ( VGA_col + 1 ) ] = tpix_shifted;
		}
...
//	*pdwPad1 = NCTL_A | NCTL_B;
	...
	*pdwPad2 = 0;
	...
	uint32_t *v_pad = (uint32_t *)XPAR_V_BTNS_BASEADDR;
	uint32_t *v_sws = (uint32_t *)XPAR_V_SWS_BASEADDR;

	const uint32_t BTN_UP = 0x1;
	const uint32_t BTN_DOWN = 0x2;
	const uint32_t BTN_RIGHT = 0x4;
	const uint32_t BTN_LEFT = 0x8;
	const uint32_t BTN_CENTER = 0x10;
//	const uint32_t BTN_A = 0x1;
//	const uint32_t BTN_B = 0x1;

	const uint32_t SW_0 = 0x01;
	const uint32_t SW_1 = 0x02;
	const uint32_t SW_2 = 0x04;
	const uint32_t SW_3 = 0x08;
	const uint32_t SW_4 = 0x10;
	const uint32_t SW_5 = 0x20;
	const uint32_t SW_6 = 0x40;
	const uint32_t SW_7 = 0x80;
...
//	xil_printf("value of v_pad: 0x%X\r\n", *v_pad);
//	xil_printf("value of v_sws: 0x%X\r\n", *v_sws);
